...
import axios, { AxiosResponse } from "config/axios-customize";
...
export const callUpdatePassword = ({
  ...
  return axios.put<IBackendRes<string>>("/api/v1/auth/update_Password", {
    currentPassword,
    newPassword,
  });
};
...
export const callForgotPassword = (
  email: string
): Promise<AxiosResponse<IBackendRes<string>, any>> => {
  return axios.put<IBackendRes<string>>("/api/v1/auth/forgotPassword", {
    email,
  });
};
...
export const callCount = () => {
  return axios.get<IBackendRes<ICount>>(`/api/v1/count`);
};
